package org.zero.utilitybox.vo.fundflow;

import lombok.AllArgsConstructor;
import lombok.Data;
import lombok.NoArgsConstructor;

/**
 * @ProjectName dabo-platform
 * @Author: zeroJun
 * @Date: 2018/7/19 11:41
 * @Description:
 */

@Data
@AllArgsConstructor
@NoArgsConstructor
public class FundFlowDetailHeadVO {

    /**
     * 记账时间
     */
    private String keepAccountTimeStr;

    /**
     * 微信支付业务单号
     */
    private String wxBusinessIdStr;

    /**
     * 资金流水单号
     */
    private String fundFlowIdStr;

    /**
     * 业务名称
     */
    private String businessNameStr;

    /**
     * 业务类型
     */
    private String businessTypeStr;

    /**
     * 收支类型
     */
    private String tranTypeStr;

    /**
     * 收支金额（元）
     */
    private String amountStr;

    /**
     * 账户结余（元）
     */
    private String accountBalanceStr;

    /**
     * 资金变更提交申请人
     */
    private String applicantStr;

    /**
     * 备注
     */
    private String remarksStr;

    /**
     * 业务凭证号
     */
    private String voucherIdStr;
}
